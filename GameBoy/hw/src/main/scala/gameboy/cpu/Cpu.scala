...
class Cpu(config: Gameboy.Configuration) extends Module {
  ...
  val initialRegisterValues = if (config.skipBootrom) {
    // Post-CGB-boot register state. The integrated core is driven in CGB
    // mode by the ZU3 host, and skipping the boot ROM must still enter the
    // cartridge at 0x0100 with the CPU state real software expects.
    // Register order: B, C, D, E, H, L, F, A, SP_hi, SP_lo, W, Z, PC_hi, PC_lo.
    Seq(0x00, 0x00, 0xFF, 0x56, 0x00, 0x0D, 0x80, 0x11, 0xFF, 0xFE, 0x00, 0x00, 0x01, 0x00)
  } else {
    Seq.fill(14)(0x00)
  }
