...
int main() {
  ...
  auto pairs = handle.malloc(sizeof(uint64_t) * kLength);
  auto *host_pairs = static_cast<uint64_t *>(pairs.getHostAddr());

  uint64_t expected = 0;
  for (uint32_t i = 0; i < kLength; ++i) {
    const uint32_t a = i + 1;
    const uint32_t b = i + 1;
    host_pairs[i] = (uint64_t)a | ((uint64_t)b << 32);
    expected += (uint64_t)a * b;
  }

  handle.copy_to_fpga(pairs);
  ...
  auto resp = DotProductCore::dot_product(0, pairs, kLength).get();
